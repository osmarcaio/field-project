...
def compute_extreme_diagnostics(point: pd.DataFrame):
    ...
    z["actual_band_gt3"] = pd.cut(z[target], bins=bins, labels=labels, right=True)
    ...
    best_col=model_cols.get("XGBoost + radar", next(iter(model_cols.values())))
    zz=z.copy()
    ...
    decomp["share_squared_error_pct"]=100*decomp["sum_SE"]/decomp["sum_SE"].sum()
    ...
    if len(moderate):
        examples.append(moderate.loc[moderate["xgb_radar_error_m"].abs().idxmax()])
        examples.append(moderate.loc[moderate["xgb_radar_error_m"].abs().idxmin()])
    # 4-5 and 5-6 examples
    for lo,hi in [(4,5),(5,6)]:
        q=z[(z[target]>lo)&(z[target]<=hi)]
        if len(q): examples.append(q.loc[q["xgb_radar_error_m"].abs().idxmax()])
    ex=pd.DataFrame(examples).drop_duplicates(subset=["prediction_time"]).copy()
    keep=["prediction_time",target]+list(model_cols.values())
    ex=ex[[c for c in keep if c in ex.columns]].sort_values("prediction_time")
...
def create_figures(outdir: Path, data: dict, extreme: tuple) -> Dict[str,Path]:
    ...
    if pm is not None:
        for _,r in pm.iterrows(): rows.append((str(r["model"]),float(r["RMSE_cm"]),"Tabular"))
    nm=data.get("neural_point")
    if nm is not None:
        for _,r in nm.iterrows(): rows.append((str(r["model"]),float(r["RMSE_cm"]),"Rede temporal"))
    if rows:
        rdf=pd.DataFrame(rows,columns=["model","RMSE_cm","family"]).sort_values("RMSE_cm")
        fig,ax=plt.subplots(figsize=(10,5.2))
        ax.barh(rdf["model"],rdf["RMSE_cm"])
        ax.invert_yaxis(); ax.set_xlabel("RMSE em 2024 (cm)"); ax.set_title("Erro global da previsao de 2 h")
        for i,v in enumerate(rdf["RMSE_cm"]): ax.text(v+0.3,i,f"{v:.1f}",va="center",fontsize=8)
        fig.tight_layout(); p=outdir/"01_rmse_global.png"; save_fig(fig,p); figs["rmse_global"]=p

    # 2 severity MAE tabular + neural
    sev=data.get("point_severity")
    frames=[]
    if sev is not None:
        s=sev[["severity_band","model","MAE_cm"]].copy(); frames.append(s)
    nsev=data.get("neural_severity")
    if nsev is not None:
        s=nsev[["severity_band","model","MAE_cm"]].copy(); frames.append(s)
    if frames:
        allsev=pd.concat(frames,ignore_index=True)
        order=["<=0","0-0.25","0.25-0.5","0.5-1","1-2","2-3",">3"]
        pv=allsev.pivot_table(index="severity_band",columns="model",values="MAE_cm",aggfunc="first").reindex(order)
        fig,ax=plt.subplots(figsize=(12,5.7)); pv.plot(kind="bar",ax=ax)
        ax.set_ylabel("MAE (cm)"); ax.set_xlabel("Variacao real em 2 h (m)"); ax.set_title("O erro cresce fortemente com a severidade")
        ax.tick_params(axis="x",rotation=20); ax.legend(fontsize=7,ncol=2); fig.tight_layout()
        p=outdir/"02_mae_por_severidade.png"; save_fig(fig,p); figs["severity"]=p

    # 3 Distribution actual >3m
    z=point[point[target]>3]
    fig,ax=plt.subplots(figsize=(9,4.8)); ax.hist(z[target],bins=np.arange(3,7.26,.25),edgecolor="white")
    ax.axvline(z[target].median(),linestyle="--",label=f"mediana {z[target].median():.2f} m")
    ax.axvline(z[target].mean(),linestyle=":",label=f"media {z[target].mean():.2f} m")
    ax.set_xlabel("Subida real nas 2 h seguintes (m)"); ax.set_ylabel("Numero de timestamps"); ax.set_title("Distribuicao dos 115 timestamps com subida > 3 m")
    ax.legend(); fig.tight_layout(); p=outdir/"03_distribuicao_gt3m.png"; save_fig(fig,p); figs["gt3_dist"]=p

    # 4 XGB+radar scatter >3m
    best_col=model_cols.get("XGBoost + radar", next(iter(model_cols.values())))
    fig,ax=plt.subplots(figsize=(6.4,6.2)); ax.scatter(z[target],z[best_col],s=22,alpha=.65)
    lo=min(3,z[best_col].min()); hi=max(z[target].max(),z[best_col].max()); ax.plot([lo,hi],[lo,hi],linestyle="--",linewidth=1)
    ax.set_xlabel("Subida real (m)"); ax.set_ylabel("Subida prevista - XGBoost + radar (m)"); ax.set_title("Casos reais > 3 m: previsto x real")
    fig.tight_layout(); p=outdir/"04_scatter_gt3m.png"; save_fig(fig,p); figs["gt3_scatter"]=p

    # 5 contribution to error
    fig,ax=plt.subplots(figsize=(9,4.8));
    x=np.arange(len(decomp)); w=.38
    ax.bar(x-w/2,decomp["share_abs_error_pct"],width=w,label="Participacao no erro absoluto")
    ax.bar(x+w/2,decomp["share_squared_error_pct"],width=w,label="Participacao no erro quadratico")
    ax.set_xticks(x,decomp["actual_band_gt3"].astype(str)); ax.set_ylabel("Participacao no erro total dos casos >3 m (%)")
    ax.set_xlabel("Faixa da subida real (m)"); ax.set_title("Os poucos casos >6 m nao explicam sozinhos o erro alto")
    ax.legend(fontsize=8); fig.tight_layout(); p=outdir/"05_decomposicao_erro_gt3m.png"; save_fig(fig,p); figs["decomp"]=p
    ...
    for key,center,label in [("feb14",pd.Timestamp("2024-02-14 01:20"),"14/02/2024"),("mar08",pd.Timestamp("2024-03-08 18:00"),"08/03/2024")]:
        q=point[(point["prediction_time"]>=center-pd.Timedelta(hours=4))&(point["prediction_time"]<=center+pd.Timedelta(hours=4))].copy()
        if len(q):
            fig,ax=plt.subplots(figsize=(11,4.8)); ax.plot(q["prediction_time"]+pd.Timedelta(hours=2),100*q[target],label="Real",linewidth=2.2)
            for name,c in model_cols.items():
                if name in ["XGBoost + radar","XGBoost normal","Duas etapas"]:
                    ax.plot(q["prediction_time"]+pd.Timedelta(hours=2),100*q[c],label=name,alpha=.9)
            ax.axhline(100,linestyle="--",linewidth=1,label="1 m")
            ax.set_ylabel("Variacao nas 2 h seguintes (cm)"); ax.set_xlabel("Horario alvo"); ax.set_title(f"Estudo de caso - {label}")
            ax.legend(fontsize=8,ncol=2); ax.tick_params(axis="x",rotation=25); fig.tight_layout(); p=outdir/f"06_case_{key}.png"; save_fig(fig,p); figs[key]=p

    # 7 alert precision/recall
    ah=data.get("alert_heads")
    if ah is not None:
        fig,ax=plt.subplots(figsize=(7.5,5));
        for _,r in ah.iterrows(): ax.scatter(r["recall_pct"],r["precision_pct"],s=90,label=str(r["model"]))
        ax.set_xlabel("Recall (%) - quanto dos eventos >=1 m foi detectado"); ax.set_ylabel("Precision (%) - quanto dos alertas estava correto")
        ax.set_title("Trade-off do classificador de alerta"); ax.legend(fontsize=8); fig.tight_layout(); p=outdir/"07_alert_precision_recall.png"; save_fig(fig,p); figs["alert_pr"]=p
    ...
    if ep is not None:
        ...
        for method,g in ep.groupby("method"):
            ax.plot(g["severity_threshold_m"],g["episode_recall_pct"],marker="o",label=method)
        ax.set_xticks([1,2,3]); ax.set_xlabel("Pico real do episodio (limiar em m)"); ax.set_ylabel("Episodios com algum alerta (%)")
        ax.set_title("Deteccao por episodio - cuidado: classificadores atuais alertam apenas >=1 m")
        ax.legend(fontsize=7,ncol=2); fig.tight_layout(); p=outdir/"08_episode_recall.png"; save_fig(fig,p); figs["episode"]=p
    ...
    if unc is not None:
        ...
        for method,g in unc.groupby("method"):
            ax.plot(g["nominal_coverage_pct"],g["empirical_coverage_2024_pct"],marker="o",label=method)
        ax.plot([75,100],[75,100],linestyle="--",linewidth=1,label="calibracao perfeita")
        ax.set_xlabel("Cobertura nominal (%)"); ax.set_ylabel("Cobertura empirica em 2024 (%)"); ax.set_title("Calibracao global dos intervalos")
        ax.legend(fontsize=8); fig.tight_layout(); p=outdir/"09_uncertainty.png"; save_fig(fig,p); figs["uncertainty"]=p
    ...
    if na is not None:
        ...
        for model,g in na.groupby("model"):
            ax.plot(g["event_threshold_m"],g["recall_pct"],marker="o",label=model)
        ax.set_xticks([1,2,3]); ax.set_xlabel("Severidade do alerta (m em 2 h)"); ax.set_ylabel("Recall (%)"); ax.set_title("Redes temporais: heads especificas para 1, 2 e 3 m")
        ax.legend(); fig.tight_layout(); p=outdir/"10_neural_alerts.png"; save_fig(fig,p); figs["neural_alerts"]=p
...
def compile_latex(tex_path: Path, pdf_path: Path) -> Tuple[bool,str]:
    ...
    if p.returncode==0 and generated.exists():
        if generated.resolve()!=pdf_path.resolve(): shutil.copy2(generated,pdf_path)
...
def build_context(data: dict, extreme: tuple):
    ...
    radar_test=radar[radar["period"]=="test_2024"] if radar is not None else None
    radar_gain=float(radar_test[radar_test["model"]=="com_radar"]["rmse_gain_vs_base_pct"].iloc[0]) if radar_test is not None and len(radar_test[radar_test["model"]=="com_radar"]) else np.nan
    ah=data.get("alert_heads")
    ep=data.get("episode_summary")
    unc=data.get("uncertainty")
    neural=data.get("neural_point")
    best_neural=neural.sort_values("RMSE_cm").iloc[0] if neural is not None and len(neural) else None
...
def make_html(report_dir: Path, figs: dict, ctx: dict, data: dict) -> Path:
    dist=ctx["dist"]; moderate=ctx["moderate"]
    best=ctx["best_point"]
    ...
    rename={"target_delta_120m":"Real (m)","pred_xgboost_normal":"XGB (m)","pred_xgboost_plus_radar":"XGB+radar (m)","pred_xgboost_ponderado":"XGB pond. (m)","pred_lightgbm":"LightGBM (m)","pred_duas_etapas":"Duas etapas (m)","prediction_time":"Horario da previsao"}
...
<section class="section"><h2>Referencias que motivam a etapa temporal</h2><ul><li>Application of temporal convolutional network for flood forecasting, Hydrology Research (2021), DOI: 10.2166/nh.2021.021.</li><li>Water Level Prediction Model Applying an LSTM-GRU Method for Flood Prediction, Water (2022), DOI: 10.3390/w14142221.</li><li>A Comprehensive Review of Methods for Hydrological Forecasting Based on Deep Learning, Water (2024), DOI: 10.3390/w16101407.</li><li>Review and Intercomparison of Machine Learning Applications for Short-term Flood Forecasting, Water Resources Management (2025), DOI: 10.1007/s11269-025-04093-x.</li></ul></section>
    ...
    path=report_dir/"relatorio_integrado_2024.html"; path.write_text(html,encoding="utf-8"); return path
...
def make_latex(report_dir: Path, figs: dict, ctx: dict, data: dict) -> Tuple[Path,Path,bool,str]:
    ...
    ex=ctx["examples"].copy()
    ex_cols=[c for c in ["prediction_time","target_delta_120m","pred_xgboost_plus_radar","pred_xgboost_normal","pred_duas_etapas"] if c in ex.columns]
    ex=ex[ex_cols].copy()
    if "prediction_time" in ex.columns:
        ex["prediction_time"]=pd.to_datetime(ex["prediction_time"]).dt.strftime("%d/%m/%Y %H:%M")
    ...
    neural_tex=""
    if neural_available:
        nr=dataframe_latex_rows(ctx["neural"],["model","MAE_cm","RMSE_cm","max_abs_error_cm"])
        neural_tex=r"""
...
\end{{document}}
"""
    tex_path=report_dir/"relatorio_integrado_2024.tex"; tex_path.write_text(tex,encoding="utf-8")
...
def main():
    ...
    report_dir=Path(args.report_dir).resolve() if args.report_dir else outputs/"reports"/"integrated_v6"
    fig_dir=report_dir/"figures"; report_dir.mkdir(parents=True,exist_ok=True)
    ...
    decomp.to_csv(report_dir/"decomposicao_erro_gt3_xgb_radar.csv",index=False)
    examples.to_csv(report_dir/"exemplos_extremos_concretos.csv",index=False)
    (report_dir/"diagnostico_gt3_resumo.json").write_text(json.dumps({**dist,"moderate_3_3_5":moderate},indent=2,default=float),encoding="utf-8")
